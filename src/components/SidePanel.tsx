    ...
    >
      <div className="flex items-center justify-between border-b border-[var(--gcal-border)] px-4 py-3">
        <div className="flex min-w-0 items-center gap-2">
          {planChildren.has(panel) ? (
            <button
              type="button"
              className="rounded-lg px-2 py-1 text-ui-sm font-semibold text-[var(--gcal-blue)] hover:bg-[var(--gcal-bg)]"
              onClick={() => setPanel('plan')}
            >
              ← Plan
            </button>
          ) : null}
          <div className="truncate text-ui-base font-semibold">{titleFor(panel)}</div>
        </div>
        <button
          type="button"
      ...
      <div className={cn('cal-scroll flex-1 overflow-auto', panel === 'trips' ? 'flex flex-col p-0' : 'p-3')}>
        {panel === 'plan' ? <PlanHubPanel /> : null}
        {panel === 'settings' ? <TripSettingsPanel /> : null}
...
function PlanHubPanel() {
  const setPanel = useTripStore((s) => s.setPanel)
  const mode = useTripStore((s) => s.mode)
  const items = [
    { id: 'checklist' as const, label: 'Checklist', icon: ListTodo, desc: 'Packing & to-dos' },
    ...
    <div className="grid gap-2">
      {items.map(({ id, label, icon: Icon, desc }) => (
        <button
          ...
          type="button"
          onClick={() => setPanel(id)}
          className="flex items-center gap-3 rounded-xl border border-[var(--gcal-border)] px-3 py-3 text-left hover:bg-[var(--gcal-bg)]"
        >
          <span className="flex size-10 items-center justify-center rounded-lg bg-[#e8f0fe] text-[var(--gcal-blue)]">
            <Icon className="size-5" />
          </span>
          <span>
            <span className="block text-ui-base font-semibold">{label}</span>
            <span className="block text-ui-sm text-[var(--gcal-muted)]">{desc}</span>
          </span>
        </button>
      ))}
      {mode !== 'edit' ? (
        <p className="mt-2 text-ui-sm text-[var(--gcal-muted)]">View-only — some actions need the edit link.</p>
      ) : null}
    </div>
  )
}
...
function TripSettingsPanel() {
  ...
  const nights = datesOk
    ? Math.max(0, differenceInCalendarDays(parseISO(endDate), parseISO(startDate)))
    : 0
  const outsideEvents =
    eventRange &&
    (eventRange.min < startDate || eventRange.max > endDate)

  async function save() {
    if (!datesOk || mode !== 'edit') return
    setBusy(true)
    try {
      await updateTrip({ name: name.trim(), startDate, endDate })
      setToast('Trip settings saved')
    } finally {
      setBusy(false)
    }
  }
  ...
  return (
    <div className="space-y-4">
      <label className="block">
        <span className="mb-1 block text-ui-sm font-medium text-[var(--gcal-muted)]">Trip name</span>
        <input
          disabled={mode !== 'edit'}
      ...
      {datesOk ? (
        <p className="text-ui-sm text-[var(--gcal-muted)]">
          {nights === 0 ? 'Same-day trip' : `${nights + 1} days · ${nights} nights`}
        </p>
      ) : (
        <p className="text-ui-sm text-[#c5221f]">End date must be on or after start.</p>
      )}
      {outsideEvents ? (
        <p className="rounded-lg bg-[#fef7e0] px-3 py-2 text-ui-sm text-[#8a5a00]">
          Some events fall outside these dates ({eventRange!.min} – {eventRange!.max}). Story and
          schedule still show them until you move events or widen dates.
        </p>
      ) : null}
      {mode === 'edit' ? (
        <button
        ...
        <p className="text-ui-sm text-[var(--gcal-muted)]">View-only — need edit link to change settings.</p>
      )}
    </div>
  )
}
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-xl bg-gradient-to-br from-[#e8f0fe] to-[#e6f4ea] p-3">
        <div className="text-ui-xl font-bold">{trip.name}</div>
      ...
      <RecapWeather events={events} />
    </div>
  )
}

function RecapWeather({ events }: { events: { location: string }[] }) {
  const [wx, setWx] = useState<{ label: string; temp: number }[] | null>(null)

  useEffect(() => {
    const locs = [...new Set(events.map((e) => e.location.trim()).filter(Boolean))].slice(0, 3)
    ...
    const targets = locs
      .map((l) => {
        ...
        const coords =
          cityCoords[key] ??
          Object.entries(cityCoords).find(([k]) => key.includes(k))?.[1]
        return coords ? { label: l.split(/[,(]/)[0].trim().slice(0, 12), ...coords } : null
      })
    ...
    void Promise.all(
      targets.map(async (c) => {
        const url = `https://api.open-meteo.com/v1/forecast?latitude=${c.lat}&longitude=${c.lon}&current=temperature_2m&temperature_unit=fahrenheit`
        const res = await fetch(url)
        const data = await res.json()
        return { label: c.label, temp: data.current?.temperature_2m as number }
      }),
    )
      .then(setWx)
      .catch(() => setWx(null))
  ...
  return (
    <div className="rounded-xl border border-[var(--gcal-border)] px-3 py-2 text-[11px] text-[var(--gcal-muted)]">
      <div className="mb-0.5 font-semibold text-[var(--gcal-text)]">Weather at your stops</div>
